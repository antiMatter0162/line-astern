...
const SPEED_ORDERS = [
  { id: "ahead_1_3", label: "1 · Ahead 1/3", fraction: 1 / 3, key: "ONE" },
  { id: "ahead_2_3", label: "2 · Ahead 2/3", fraction: 2 / 3, key: "TWO" },
  { id: "ahead_standard", label: "3 · Ahead Standard", fraction: 0.75, key: "THREE" },
  { id: "ahead_full", label: "4 · Ahead Full", fraction: 0.9, key: "FOUR" },
...
function create() {
  ...
  this.add.rectangle(6400, 4000, 12800, 8000, 0x06345a);
...
function createSpeedHud(scene) {
  ...
  speedHudBackdrop = scene.add.rectangle(
    0,
    startY - 10,
    SPEED_ORDERS.length * spacing + 10,
    50,
    0x000000,
    0.55,
  )
  ...
  speedHudButtons = SPEED_ORDERS.map((order, index) =>
    scene.add.text(startX + index * spacing, startY, order.label, {
      fontSize: "15px",
      color: "#ffffff",
...
function updateSpeedHud() {
  ...
  console.log("[speed HUD] selectedShips:", selectedShips.length, "-> visible:", hasSelection);
...
function issueMoveOrder(x, y, append) {
  ...
  selectedShips.forEach((ship) => {
    ...
    if (!ship.pathLine) ship.pathLine = scene.add.graphics().setDepth(1);
    updatePathLine(ship);
  });
}
